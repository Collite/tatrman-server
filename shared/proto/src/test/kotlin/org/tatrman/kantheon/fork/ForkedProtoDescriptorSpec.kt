    ...
    StringSpec({
        ...
        val forked: List<Pair<Descriptors.FileDescriptor, String>> =
            listOf(
                ...
                // (kadmos.v1 + common.v1) — included here so the com/tatrman-free
        ...
        // (b) no forked descriptor (transitively) depends on a com/tatrman/* file.
        forked.forEach { (fd, _) ->
            "${fd.name} has no com/tatrman dependency" {
                transitiveDeps(fd).forEach { dep ->
                    dep.name shouldNotStartWith "com/tatrman/"
                }
            }
        }
