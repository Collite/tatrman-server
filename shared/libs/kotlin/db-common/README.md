...
HikariCP + Exposed bootstrap for MSSQL/Postgres, with HOCON-driven `DatabaseConfig` and a `DatabaseConnection` lifecycle owner. Used by `workers/ttr-worker-mssql` (MSSQL), `services/ttr-validate` and `services/ttr-query` (Postgres).
