...
def _gen_proto():
    """`scripts/gen_proto.py` as an imported module.

    Loaded and called rather than run as `__main__`: the script ends in
    `raise SystemExit(main())`, and a SystemExit(0) escaping a build hook exits
    the backend process cleanly enough that hatchling never writes its result
    file — the front-end then fails with a missing `build_wheel.txt` and no
    mention of this hook at all.

    Importing it also gives us its `PROTO_ROOT`, so this file does not carry a
    second copy of the `../../../../shared/proto` derivation to drift from.
    """
...
class CustomBuildHook(BuildHookInterface):
    ...
    def initialize(self, version: str, build_data: dict) -> None:
        ...
        # An EDITABLE install is not a distribution, and must not need the proto
        # tree. `services/nlp`'s Dockerfile is the case that proves it: its deps
        # stage copies `shared/libs/python/ttr-nlp` and runs `uv sync --frozen`
        # long before `shared/proto` is copied in, so generating here failed the
        # whole image build — and it failed for a consumer that never wanted the
        # bundled stubs, because a checkout that path-depends on this wheel has
        # its own `generated/` tree on `pythonpath` (that is where `ttrnlp.proto`
        # looks first). Bundling is a shipping concern; skip it for editables.
        if version == "editable":
            return

        # `grpc_tools` comes from `[build-system] requires`, so this runs in the
        # isolated build env without the dev group being installed.
        gen_proto = _gen_proto()
        if not gen_proto.PROTO_ROOT.exists():
            raise RuntimeError(
                f"cannot build a ttr-nlp wheel without the proto sources at "
                f"{gen_proto.PROTO_ROOT} — `shared/proto` is the canonical source "
                "for the `org.tatrman.*.v1` stubs this wheel ships (NL-16). Build "
                "from a full checkout."
            )

        if not STUB_MARKER.exists():
            rc = int(gen_proto.main())
            if rc:
