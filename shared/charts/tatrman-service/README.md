# `tatrman-service` — shared Helm library chart
...
```
<module>/k8s/
├── Chart.yaml            # type: application + a dependency on tatrman-service
├── values.yaml           # the module's defaults (image, ports, resources, env inputs)
└── templates/
    ├── deployment.yaml   # {{ include "tatrman-service.deployment" . }}
    ├── service.yaml      # {{ include "tatrman-service.service" . }}
...
```yaml
dependencies:
  - name: tatrman-service
    version: 0.1.0
    repository: "file://../../../shared/charts/tatrman-service"
```
...
| Template | Emits |
|---|---|
| `tatrman-service.deployment` | backend Deployment (Kotlin/Ktor · agent · worker · Python) |
| `tatrman-service.service` | backend ClusterIP Service (`http`; `grpc` gated on `.Values.ports.grpc`) |
| `tatrman-service.helpers` (`.name`/`.fullname`/`.labels`/`.selectorLabels`/`.image`) | naming + labels, keyed off `.Chart.*` |
| `tatrman-service.fe-deployment` / `.fe-service` / `.fe-configmap` / `.httproute` | FE-nginx shape (see `frontends/iris/k8s`) |
...
`tatrman-service.volumeMounts` and `tatrman-service.volumes` default to empty.
