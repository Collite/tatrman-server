...
{{- define "tatrman-service.fe-configmap" -}}
apiVersion: v1
kind: ConfigMap
metadata:
  name: {{ include "tatrman-service.fullname" . }}-config
  labels:
...
{{- define "tatrman-service.fe-deployment" -}}
apiVersion: apps/v1
kind: Deployment
metadata:
  name: {{ include "tatrman-service.fullname" . }}
  labels:
    {{- include "tatrman-service.labels" . | nindent 4 }}
spec:
  ...
  template:
    metadata:
      annotations:
        ...
        checksum/config: {{ printf "%s\n" (include "tatrman-service.fe-configmap" .) | sha256sum }}
    ...
    spec:
      ...
      containers:
        - name: {{ .Chart.Name }}
          image: {{ include "tatrman-service.image" . | quote }}
...
{{- define "tatrman-service.fe-service" -}}
apiVersion: v1
kind: Service
metadata:
  name: {{ include "tatrman-service.fullname" . }}
...
spec:
  ...
  selector:
    {{- include "tatrman-service.selectorLabels" . | nindent 4 }}
  ports:
    - name: http
...
{{- define "tatrman-service.httproute" -}}
