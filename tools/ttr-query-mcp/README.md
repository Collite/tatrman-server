...
**theseus-mcp** — the agent-facing MCP edge for **Theseus**. Exposes the
...
The resolved roles become `PipelineContext.auth_roles` and reach Argos
unchanged (bearer-roles, contracts §3) — verified end-to-end by
`RunQueryFullChainComponentSpec` (run_query → real in-process Theseus → mocked
Proteus/Argos/Kyklop/worker → Arrow → JSON rows).
...
(`theseus.query:v1`, `theseus.compile:v1`) and register with capabilities-mcp at
startup (warn-and-continue; opt in with `CAPABILITIES_MCP_URL`).

## Status (Stage 3.5)

Port: HTTP 7307 (MCP `/mcp` + `/health` + `/ready`). Upstreams: Theseus (gRPC
7306) + Proteus/Argos/Ariadne for compile/decoration. Tag: `theseus-mcp/v0.1.0`.
