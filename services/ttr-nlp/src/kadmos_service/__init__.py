...
Dockerfile launches the app via `uvicorn kadmos_service.api.routes:app`
which bypasses main.py entirely — any basicConfig there would never
run in production. Importing any submodule (`kadmos_service.api.routes`,
`kadmos_service.engines.stanza_engine`, …) walks through this package
...
logging.getLogger("kadmos-service").setLevel(_log_level)
logging.getLogger("kadmos_service").setLevel(_log_level)
logging.getLogger(__name__).info(
    "NLP service log level set to %s (override via LOG_LEVEL env)",
    _log_level_name,
)

from kadmos_service.api.routes import create_app  # noqa: E402
