...
from kadmos_service.pipeline.orchestrator import Orchestrator
from otel_config import setup_opentelemetry, instrument_fastapi

logger = logging.getLogger("kadmos-service")
...
def create_app() -> FastAPI:
    ...
    otel_port = int(os.getenv(
        ...
        "4317" if os.getenv("KADMOS_SERVICE_OTEL_PROTOCOL", "grpc").lower() == "grpc"
        else os.getenv("OTEL_EXPORTER_OTLP_HTTP_PORT", "4318")
    ))
    otel_protocol = os.getenv("KADMOS_SERVICE_OTEL_PROTOCOL", "grpc").lower()
    ...
    request_counter = meter.create_counter(
        name="kadmos_request_total",
        description="Total Kadmos NLP requests",
        unit="1",
    )
    request_duration = meter.create_histogram(
        name="kadmos_request_duration_seconds",
        description="Kadmos NLP request duration in seconds",
        unit="s",
    )

    app = FastAPI(
        title="Kadmos NLP Service",
