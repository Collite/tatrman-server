...
/**
 * Resolves the **effective** role set Validate drives RLS + admin-bypass with. Identity is always
 * the bearer-trusted `PipelineContext.user_id` / `auth_roles` (resolved at the query-mcp edge);
 ...
 *  - [WhoisRoleSource]  — opt-in (`validate.roleSource = whois`): enriches the bearer floor with the
...
/**
 * Thrown when a role-enrichment source cannot be reached. Validate **fails closed** on this
