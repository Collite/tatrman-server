...
class ConnectionRegistry private constructor(
    ...
    companion object {
        ...
        /**
         * Build from a YAML string, resolving each `${VAR}` credential token through
         * [resolver] — the CH-D3 open port. Default = [ConnectionSecretResolver.discover]
         * (the `ServiceLoader`-registered provider, or env-var binding when none is
         * registered). The SPI-backed resolution (`fromSecrets`, `TransferSecretInjector`,
         * the hall's `TTR_CONN_*` least-exposure store) is the platform adapter's job at
         * CH-P2 — it registers a provider on this port; open Charon carries no `cz.tatrman`
         * dependency. Resolved material lands only in the in-memory handles (never a config
         * file, run-store, log, or the wire).
         */
        fun fromYaml(
            yaml: String,
            resolver: ConnectionSecretResolver = ConnectionSecretResolver.discover(),
        ): ConnectionRegistry = ConnectionRegistry(parse(yaml, resolver))

        /** Map-backed overload — a caller holding an already-resolved env map (tests,
         *  and the legacy `${ENV}` callers). */
        fun fromYaml(
            yaml: String,
            env: Map<String, String>,
        ): ConnectionRegistry = fromYaml(yaml, ConnectionSecretResolver { env[it] })
        ...
        fun fromFile(
            ...
            if (path.isFile) {
                fromYaml(path.readText(), resolver)
            } else {
                log.info("no connection registry at {} — DB edges disabled", path)
                ConnectionRegistry(emptyMap())
            }

        /** Map-backed overload (tests + legacy `${ENV}` callers). */
        fun fromFile(
            path: java.io.File,
            env: Map<String, String>,
        ): ConnectionRegistry = fromFile(path, ConnectionSecretResolver { env[it] })
                ...
                doc.connections.mapNotNull { c ->
                    ...
                    try {
                        ConnectionHandle(
                            id = c.id,
                            dialect = dialect,
                            jdbcUrl = substitute(c.jdbcUrl, resolver),
                            username = substitute(c.username, resolver),
                            password = substitute(c.password, resolver),
                            allow =
                                AllowList(
                                    read = c.allow.read,
                                    write = c.allow.write,
        ...
        /** Replace `${VAR}` tokens with [resolver].resolve(VAR); an unresolved var
         *  throws [UnresolvedCredentialException] so the connection is skipped (never
         *  loaded with a blank credential). */
        private fun substitute(
            raw: String,
            resolver: ConnectionSecretResolver,
        ): String =
            Regex("\\$\\{([A-Za-z_][A-Za-z0-9_]*)}").replace(raw) { m ->
                val key = m.groupValues[1]
                resolver.resolve(key) ?: throw UnresolvedCredentialException(key)
            }
    }
}
