...
/**
 * The shared cross-engine fingerprint anchor (reference/scalars/list/map.arrow + fingerprints.json).
 * PL-P3.S1.T2: on the platform these live in this module's test resources (`src/test/resources/fingerprints`)
 * rather than kantheon's repo-root `shared/testdata/` — the pins are unchanged, so the cross-engine
 * agreement Charon verifies against is identical to the donor's.
 */
private fun locateFixtureDir(): File {
    val url =
        object {}.javaClass.classLoader.getResource("fingerprints")
            ?: error("could not locate the 'fingerprints' fixture dir on the test classpath")
