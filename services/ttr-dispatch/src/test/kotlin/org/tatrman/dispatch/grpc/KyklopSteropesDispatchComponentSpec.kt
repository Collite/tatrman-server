...
/**
 * Fork Stage 3.4 T4 — Kyklop ⇄ Steropes dispatch component test (in-process worker).
 *
 * Steropes is the stateful Polars worker: it advertises `supports_stateful_sessions = true`
 * and no `supported_connections` (a DataFrame engine, not a connection-backed one), so
 * Kyklop routes workspace-rooted plans to it by session, sticky-pinning a `session_id` to
 * one pod. This exercises the stateful path the MSSQL worker (Brontes) doesn't have:
 *
 *   1. a WorkspaceRef plan + session routes to Steropes; Arrow IPC streams back intact;
 ...
 */
class KyklopSteropesDispatchComponentSpec :
    StringSpec({
        ...
        fun statefulSteropes(endpoint: String): SteropesStub =
            SteropesStub(
                endpoint = endpoint,
                execute = {
                    flow {
        ...
        "workspace-rooted plan + session routes to Steropes and Arrow IPC streams back intact" {
            val steropes = statefulSteropes("steropes:7301")
            val req =
                DispatchRequest
                    .newBuilder()
            ...
            val out = svc(steropes).dispatch(req).toList()
            ...
            steropes.received.size shouldBe 1
        }

        "stage-then-read: assign_to_workspace forwarded and the session sticks to one worker" {
            val a = statefulSteropes("steropes-a:7301")
            val b = statefulSteropes("steropes-b:7301")
            val svc = svc(a, b)
