...
/**
 * Fork Stage 3.3 T6 — Kyklop ⇄ Brontes dispatch component test (in-process worker).
 *
 * Kyklop receives a validated fixture plan, matches it to Brontes by capability
 * (engine "mssql" serving connection "df-test"), and streams the worker's Arrow IPC
 * batches back through itself. Kyklop's contract is pass-through fidelity: it must
 ...
 */
class KyklopBrontesDispatchComponentSpec :
    StringSpec({

        // Two distinct Arrow IPC frames, as Brontes would emit them: a first batch
        ...
        // Brontes: an in-process MSSQL worker serving df-test, streaming two real frames.
        val brontes =
            BrontesStub(
        ...
        fun svc(): KyklopServiceImpl {
            ...
            registry.seed(
                listOf(
                    WorkerEntry(
                        endpoint = "brontes:7296",
                        roleHint = "mssql",
                        client = brontes,
                        capabilities = brontes.capabilities,
        ...
        "validated plan routes to Brontes and Arrow IPC streams back through Kyklop intact" {
            ...
            // Kyklop annotated the routing decision on the first batch (its only addition).
