...
"""Generate nlp's Python proto stubs (messages + gRPC service).

nlp owns its `org.tatrman.{nlp,common}.v1` stubs rather than consuming the
...
compiles the two files nlp needs (nlp.proto + its common import) with
grpcio-tools' bundled protoc + grpc_python plugin.

Run:  uv run python scripts/gen_proto.py
Output: services/nlp/generated/org/tatrman/{nlp,common}/v1/*.py(i)
"""
...
# services/nlp -> repo root (tatrman-server)
