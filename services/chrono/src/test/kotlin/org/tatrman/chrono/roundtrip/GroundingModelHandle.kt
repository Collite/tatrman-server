...
class GroundingModelHandle(
    ...
    companion object {
        ...
        /** Fact `Sale(date, due)` + period `AccountingPeriod(start_date, end_date, period)`. */
        fun saleModel(): GroundingModelHandle =
            GroundingModelHandle(
                listOf(
                    dbTable("Sale", listOf("date" to SurfaceType.DATETIME, "due" to SurfaceType.DATETIME)),
                    dbTable(
                        "AccountingPeriod",
                        listOf(
                            "start_date" to SurfaceType.DATETIME,
                            "end_date" to SurfaceType.DATETIME,
                            "period" to SurfaceType.TEXT,
                        ),
                    ),
                ),
            )
