 ...
 * — the single source of truth (contracts §1.1 "derived, not duplicated"). A8.5 round-trips the
 * output back through the Translator to prove the two agree.
 *
 * Column names are double-quoted (`t."date"`): real grounding columns collide with SQL reserved
 * words (`date`, `period`, …), so an unquoted preview would not parse under the Translator's
 * `Lex.MYSQL_ANSI` (double-quote quoting, casing unchanged, case-insensitive). [quote] the entity
 * name where the caller renders one too.
 */
object SqlRenderer {
    /** Double-quote an identifier so reserved words (`date`, `period`, `order`, …) parse. */
