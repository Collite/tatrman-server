...
/**
 * WS-C1 T3 — Validate's RLS policy decision, **enforced against a real Postgres**. Where the unit
 ...
 *
 * The predicate's column and literal are read back **from Validate's decision** (not hardcoded in the
 ...
 */
@Tags("component")
class ValidateRlsComponentSpec :
    StringSpec({
        ...
        "the tenant_isolation predicate denies cross-tenant rows and admits same-tenant rows on real Postgres" {
            Containers.postgres().use { pg ->
                ...
                connect(pg.jdbcUrl, pg.username, pg.password).use { c ->
                    // Two tenants' rows in one physical table — the warehouse shape Validate guards.
                    ...
                    // Tenant-7: Validate emits `tenant_id = 'tenant-7'`; enforcing it admits only alice+amir.
                    ...
                    // Tenant-9: symmetric — Validate's decision restricts to bob+bella, denies tenant-7's rows.
...
/**
 * Ask Validate to evaluate policies for [userId] and render its single tenant-isolation `eq` predicate
 * as a SQL `WHERE` fragment — the trivial unparse the worker does downstream, driven entirely by
 * Validate's emitted column + literal.
 */
