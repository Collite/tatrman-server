...
# Vendor the tatrman-service library into every per-service chart, then build the
...
# Every per-service chart is a `<component>/k8s` dir that depends on tatrman-service.
mapfile -t CHARTS < <(find services workers tools infra -path '*/k8s/Chart.yaml' \
  -exec dirname {} \; | sort)

echo "→ vendoring tatrman-service into ${#CHARTS[@]} per-service charts"
for c in "${CHARTS[@]}"; do
