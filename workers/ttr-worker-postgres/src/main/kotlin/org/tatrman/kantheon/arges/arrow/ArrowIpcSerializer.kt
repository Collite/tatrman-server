package org.tatrman.kantheon.arges.arrow
...
object ArrowIpcSerializer {
    /**
     * Returns the IPC bytes for one batch (schema-prefixed streaming form). The
     * v1 implementation always emits the schema header per batch; consumers parse
     * the stream forwards regardless. Omitting the schema on later batches is a
     * v1.5+ refinement.
     */
     ...
     * implementation-independent logical-schema string. The shared fixture set
     * (`shared/testdata/fingerprints/`) pins all worker implementations together.
     */
