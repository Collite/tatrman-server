 ...
 * contract Postgres relies on. The seed runs as the superuser, which bypasses RLS, so inserts need
 * no GUC.
 */
object PostgresPgFixture {
    ...
    fun provision(superuser: Connection) {
        superuser.createStatement().use { st ->
            ...
            // The non-owner login role Postgres connects as — subject to RLS (not an owner/superuser).
