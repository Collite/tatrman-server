...
/**
 * Drives the real Arges worker pipeline against a live Postgres, exactly as the unit/component
 * Brontes spec does: a faked translator stands in for Proteus (returns the SQL directly), the rest
 * of the path — pool → `SET LOCAL app.tenant_id` → JDBC → Arrow IPC → ResultBatch — is production code.
 */
object ArgesComponentSupport {
    private val limits =
    ...
    /** A read-only, tenant-enforcing pool that connects as the non-owner [ArgesPgFixture.READONLY_ROLE]. */
    fun pool(
        jdbcUrl: String,
        database: String,
            ...
            mapOf(
                "pg-midas" to
                    ConnectionConfig(
                        id = "pg-midas",
                        jdbcUrl = jdbcUrl,
                        username = ArgesPgFixture.READONLY_ROLE,
                        password = ArgesPgFixture.READONLY_PW,
                        database = database,
