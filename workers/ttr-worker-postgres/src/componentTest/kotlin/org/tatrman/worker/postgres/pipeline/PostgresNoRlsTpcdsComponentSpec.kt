 ...
 * `requires-tenant-id=false` (Dispatch's `pg-tpcds`, a plain analytical warehouse with no tenant
 ...
 *
 * Postgres is native multi-arch, so no `@EnabledIf(CiOnly)` gate (unlike the Mssql/MSSQL specs).
 */
@Tags("component")
class PostgresNoRlsTpcdsComponentSpec :
    StringSpec({
        ...
        "a pg-tpcds (requires-tenant-id=false) scan runs with NO tenant and NO SET LOCAL, numeric at (20,4) boundary" {
            Containers.postgres().use { pg ->
                ...
                // Postgres sees behind the `pg-tpcds` profile. Seed the numeric(20,4) boundary value.
                connect(pg.jdbcUrl, pg.username, pg.password).use { su ->
                    su.createStatement().use { st ->
                        ...
                        // The non-owner login role Postgres connects as for pg-tpcds — SELECT only,
...
/** A faked Translate that returns the TPC-DS-subset SELECT directly (the unit/Mssql convention). */
