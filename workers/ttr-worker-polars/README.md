...
**Steropes** — the Polars worker for kantheon, the second of the Kyklops (the
individually-named workers Kyklop dispatches to). Python + Polars + gRPC +
...
Steropes implements the generic `org.tatrman.worker.v1.WorkerService` contract
(`Execute` / `GetCapabilities` / `GetStatus`) — no Steropes-specific proto. It
...
```bash
uv sync
uv run python -m workers_steropes.main
```

Default ports: HTTP 7300 / gRPC 7301. Tests: `just test-py workers/steropes`.

## Layout

* `src/workers_steropes/` — package source (`grpc_service`, `converter`,
  `workspace`, `config`, `probes`, `main`).
* `conf/application.conf` — HOCON config under the `workers-steropes {}` namespace.
...
the Python reference) is the cross-engine schema-identity check shared by Brontes
(Kotlin) and Steropes (Python). The pinned reference fixtures live in
`shared/testdata/fingerprints/` (Stage 3.4 T2); Steropes's pytest recomputes
...
`theseus.query:v1`, not on a worker manifest (workers carry no capability manifest).
...
* gRPC stubs are not auto-generated; the worker registers handlers via
  ...
  message types from `org.tatrman.{worker,plan}.v1` (+ `ariadne.v1` /
  `kantheon.common.v1` for status + Rule-6 messages).
