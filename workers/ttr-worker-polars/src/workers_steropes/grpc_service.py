...
from workers_steropes.probes import execute_duration_seconds, executes_total
from workers_steropes.workspace import WorkspaceCapExceeded, WorkspaceStore

logger = logging.getLogger("workers_steropes.grpc_service")
...
_tracer = trace.get_tracer("workers-steropes")
...
class WorkerService:
    ...
    def __init__(self, cfg: WorkersSteropesConfig, store: WorkspaceStore) -> None:
        self._cfg = cfg
        self._store = store
    ...
    async def execute(
        ...
        with _tracer.start_as_current_span("workers-steropes.Execute") as root_span:
            ...
            try:
                with _tracer.start_as_current_span("workers-steropes.convert"):
                    lf = await converter.convert(request.plan)
                with _tracer.start_as_current_span("workers-steropes.collect"):
                    df = lf.collect()
            ...
            if request.assign_to_workspace:
                try:
                    with _tracer.start_as_current_span("workers-steropes.workspace_put") as ws_span:
            ...
            with _tracer.start_as_current_span("workers-steropes.serialize") as ser_span:
                table = df.to_arrow()
                batch_size_rows = (
    ...
    async def import_data_frame(
        ...
        """
        with _tracer.start_as_current_span("workers-steropes.ImportDataFrame") as span:
...
def make_capabilities(cfg: WorkersSteropesConfig) -> worker_pb2.GetCapabilitiesResponse:
...
def _bindings_to_python(bindings) -> dict[str, object]:
    ...
    `_literal_to_python`.
    """
    from workers_steropes.converter import _literal_to_python
...
def _schema_fingerprint(schema: pa.Schema) -> str:
    ...
    ResultBatch.schema_fingerprint Steropes stamps is byte-identical to what
    Brontes (Kotlin) and Charon (Integrity.kt) compute for the same schema —
