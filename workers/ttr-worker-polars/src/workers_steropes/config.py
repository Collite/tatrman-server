...
def load_config(path: str | Path | None = None) -> WorkersSteropesConfig:
    """Load HOCON config from ``path`` (or ``WORKERS_STEROPES_CONFIG`` env, or default)."""
    config_path = (
        Path(path)
        if path is not None
        else Path(os.environ.get("WORKERS_STEROPES_CONFIG", "conf/application.conf"))
    )
    ...
    tree = ConfigFactory.parse_file(str(config_path)).get_config("workers-steropes")
    return _from_tree(tree)


def _from_tree(tree: ConfigTree) -> WorkersSteropesConfig:
