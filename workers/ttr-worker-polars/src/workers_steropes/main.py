...
Boot sequence:
  ...
  2. OTEL setup via the shared ``otel_config.setup_opentelemetry`` (DF-P01,
     Phase 06 C4) when ``workers-steropes.telemetry.enabled`` is true.
...
from workers_steropes.config import WorkersSteropesConfig, load_config
from workers_steropes.grpc_service import WorkerService, build_worker_service_handlers
from workers_steropes.probes import (
    make_probes_app,
    workspace_bytes,
    workspace_dfs,
...
_SERVICE_NAME = "workers-steropes"
...
async def _run_http(cfg: WorkersSteropesConfig, ready: asyncio.Event, workspace_stats: Any) -> None:
...
async def _amain() -> None:
    ...
    # overlay sets OTEL_ENABLED_STEROPES=true + OTEL_EXPORTER_OTLP_ENDPOINT.
    if cfg.telemetry.enabled:
        setup_opentelemetry(
            service_name="workers-steropes",
