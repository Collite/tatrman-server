...
the tatrman-service library's **backend** Deployment/Service shape + a Gateway API HTTPRoute
...
Chosen the **backend** templates (`tatrman-service.deployment` / `.service`) plus
`templates/httproute.yaml` (`tatrman-service.httproute`) — Backstage reads its config from env
